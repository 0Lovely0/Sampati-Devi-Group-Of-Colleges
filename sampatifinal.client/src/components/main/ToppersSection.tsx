  ...
  return (
    <section className="py-10 bg-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 mb-12 flex justify-between items-end">
        ...
        <Link 
          to="/topperpage" 
          className="hidden md:flex items-center gap-2 font-bold text-indigo-600 hover:text-indigo-800 transition-colors"
        >
          View All Toppers <span>→</span>
        </Link>
      </div>
      ...
      <div className="relative flex overflow-hidden">
        <div className="flex animate-scroll gap-8 px-8">
          {[...toppers, ...toppers].map((topper, index) => (
            <div key={index} className="group relative w-64 flex-shrink-0 cursor-pointer">
              <div className="rounded-3xl overflow-hidden shadow-lg border border-slate-100 transition-all duration-300 group-hover:shadow-indigo-200 group-hover:scale-105">
                <img src={topper.image} alt={topper.name} className="w-full h-80 object-cover" />
      ...
      <div className="md:hidden mt-8 px-6 text-center">
        <Link to="/topperpage" className="inline-block w-full py-4 bg-slate-900 text-white rounded-2xl font-bold">
          View All Toppers
        </Link>
      </div>
    </section>
