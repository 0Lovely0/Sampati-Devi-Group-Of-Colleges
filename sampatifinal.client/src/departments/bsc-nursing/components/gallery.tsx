  ...
  useEffect(() => {
    const fetchGalleries = async () => {
      try {
        ...
        const bscNursingImages = (data || []).filter((item) =>
          item.departments?.some(
            (dept) => dept.departmentId === 1
          )
        );

        setGalleryItems(bscNursingImages);
      } catch (error) {
    ...
    >
      ...
      >
        ...
        <p className="text-white/80 mt-4 max-w-2xl mx-auto text-lg">
          Explore moments from academic activities, clinical
          training, events, celebrations, and achievements of the
            ...
            {galleryItems.length === 0 ? (
              <div className="bg-white rounded-3xl p-10 text-center shadow-lg max-w-3xl mx-auto">
                ...
                <p className="text-slate-500 mt-2">
                  No B.Sc Nursing gallery images are available at
                  the moment.
                </p>
              </div>
            ) : (
              <>
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {galleryItems.slice(0, 10).map((item) => (
                    <div
                      ...
                      className="group relative h-32 overflow-hidden rounded-2xl bg-white border border-teal-100 shadow-sm cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
                    >
                      <img
                ...
                <div className="mt-12 flex justify-center">
                  <Link
                    to="/programs/bsc-nursing/gallery"
