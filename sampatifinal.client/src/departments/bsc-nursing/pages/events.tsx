...
const EventsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        const bscNursingEvents = data
          .filter((event: any) =>
            event.departments?.some(
              (dept: any) =>
                dept.departmentId ===
                BSC_NURSING_DEPARTMENT_ID
            )
          )
          .sort(
            (a: any, b: any) =>
        ...
        const mapped: EventUI[] = bscNursingEvents.map(
          (e: any) => ({
            id: e.eventId,
            title: e.title,
            desc: e.description,
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F8FFFE] font-bold tracking-widest text-[#0F766E] text-xs">
        LOADING B.SC NURSING EVENTS...
      </div>
    );
  }
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#F8FFFE]">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-[#0F766E]">
            No B.Sc Nursing Events Available
          </h2>
  ...
  return (
    <div className="min-h-screen bg-[#F8FFFE] pb-20">
      ...
      <div className="bg-[#0F766E] py-20 px-4 text-center">
        ...
        <p className="text-[#CCFBF1] mt-6 max-w-xl mx-auto text-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {events.map((event) => (
            <div
              ...
              className="group bg-white rounded-3xl border border-[#CCFBF1] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-300 cursor-pointer hover:-translate-y-2"
            >
              ...
              <div className="p-6">
                <span className="text-[10px] font-black text-[#0F766E] uppercase tracking-widest bg-[#CCFBF1] px-3 py-1 rounded-full">
                  {event.date}
                </span>

                <h2 className="text-lg font-black text-slate-900 mt-4 leading-tight group-hover:text-[#0F766E] transition">
                  {event.title}
                </h2>
      ...
      {selectedEvent && (
        <div
          className="fixed inset-0 z-[9999] flex items-center justify-center bg-[#0F766E]/90 p-4 backdrop-blur-sm"
        ...
        >
          ...
          >
            ...
            <div className="p-8">
              ...
              <div className="mt-6 inline-flex items-center px-4 py-2 rounded-full bg-[#CCFBF1] text-[#0F766E] text-xs font-bold">
