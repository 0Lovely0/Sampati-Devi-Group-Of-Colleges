...
export const MPHWNoticeBoardPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchNotices = async () => {
      try {
        ...
        setNotices(mphwNotices);

        if (mphwNotices.length > 0) {
          setSelectedNotice(mphwNotices[0]);
        }
      } catch (error) {
        console.error("MPHW notices load error:", error);
      } finally {
        setLoading(false);
      }
    };
    ...
    >
      ...
      <div className="w-full mx-auto px-4 -mt-8">
        ...
        >
          ...
          <input
            type="text"
            placeholder="Search MPHW notices..."
                ...
                <>
                  ...
                  {selectedNotice.notification_file && (
                    <button
                      ...
                      className="mt-8 flex items-center justify-center gap-2 w-full py-4 rounded-xl font-bold bg-white"
                      style={{ color: MPHW_THEME.primary }}
                    >
                      <FileText size={18} />
                      View Document
...
export default MPHWNoticeBoardPage;