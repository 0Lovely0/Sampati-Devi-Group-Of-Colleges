...
export const PharmacyNoticeBoardPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchNotices = async () => {
      try {
        ...
        const pharmacyNotices = data
          .filter((n) => n.notification_status === true)
          .filter((n) =>
            n.departments?.some(
              (d: any) => Number(d.departmentId) === PHARMACY_DEPT_ID
            )
          )
          .sort(
            (a, b) =>
        ...
        setNotices(pharmacyNotices);

        if (pharmacyNotices.length > 0) {
          setSelectedNotice(pharmacyNotices[0]);
        }
      } catch (error) {
        console.error("Pharmacy notices load error:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    ...
    >
      ...
      <div className="w-full mx-auto px-4 -mt-8">
        {/* SEARCH */}
        <div
          className="bg-white p-2 rounded-2xl shadow-xl flex items-center gap-3 border mb-10"
        ...
        >
          ...
          <input
            type="text"
            placeholder="Search Pharmacy notices..."
                ...
                <>
                  ...
                  {selectedNotice.notification_file && (
                    <button
                      ...
                      className="mt-8 flex items-center justify-center gap-2 w-full py-4 rounded-xl font-bold bg-white text-[#7C3AED]"
                    >
                      <FileText size={18} />
                      View Document
                      <ArrowRight size={18} />
...
export default PharmacyNoticeBoardPage;