...
export const PharmacyToppersGallery: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Pharmacy toppers load error:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  // ✅ FILTER: Pharmacy ONLY
  const pharmacyToppers = (toppers || []).filter((t: any) =>
    t?.departments?.some(
      (d: any) =>
        Number(d?.departmentId) === PHARMACY_ID ||
        d?.departmentName?.toLowerCase()?.trim() === "pharmacy"
    )
  );
    ...
    >
      ...
      >
        ...
        <p className="text-white/70 mt-4 max-w-xl mx-auto text-xs">
          Celebrating excellence and academic achievement in the Pharmacy
          Department.
        </p>
      </div>
      ...
      <div className="w-full mx-auto px-3 -mt-10">
        {loading ? (
          <div className="h-64 flex items-center justify-center">
            <Loader text="Loading Pharmacy toppers..." />
          </div>
        ) : pharmacyToppers.length === 0 ? (
          <div className="text-center text-slate-600 text-sm py-20">
            No Pharmacy toppers found.
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {pharmacyToppers.map((t) => (
              <div
                key={t.topperId}
                onClick={() => setSelected(t)}
                className="group bg-white rounded-xl border border-[#DDD6FE] overflow-hidden shadow-sm cursor-pointer transition-all hover:-translate-y-1 hover:shadow-md"
              >
                {/* IMAGE */}
...
export default PharmacyToppersGallery;