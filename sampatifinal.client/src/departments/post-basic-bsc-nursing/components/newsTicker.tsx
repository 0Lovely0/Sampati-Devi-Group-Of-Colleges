...
const POST_BASIC_DEPARTMENT_ID = 2; // Post Basic B.Sc Nursing
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        ...
        const postBasicNews = data
          .filter((item) =>
            item.departments?.some(
              (dept) => dept.departmentId === POST_BASIC_DEPARTMENT_ID
            )
          )
          .sort(
            (a, b) =>
        ...
        setNews(postBasicNews);
      } catch (error) {
        console.error("Failed to fetch Post Basic B.Sc Nursing news:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <>
      ...
      >
        ...
        {/* TICKER */}
        <Link
          to="/programs/post-basic-bsc-nursing/news"
        ...
        >
          ...
          {loading ? (
            <div className="h-9 flex items-center px-4 text-blue-100 text-xs animate-pulse">
              Loading Post Basic B.Sc Nursing updates...
            </div>
          ) : news.length > 0 ? (
            <div className="flex whitespace-nowrap animate-marquee group-hover:[animation-play-state:paused]">
              {[...news, ...news].map((item, index) => (
                ...
                >
                  ...
                  <span className="font-medium hover:text-[#DBEAFE] transition-colors text-lg">
                    {item.news_subject}
                  </span>
                </div>
              ))}
            ...
            <div className="h-9 flex items-center px-4 text-blue-100 text-xs italic">
