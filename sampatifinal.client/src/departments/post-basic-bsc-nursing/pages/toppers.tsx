...
const POST_BASIC_BSC_NURSING_ID = 2; // adjust if needed
...
export const BscNursingToppersGallery: React.FC = () => {
  ...
  // ✅ POST BASIC FILTER
  const postBasicToppers = (toppers || []).filter((t: any) =>
    t?.departments?.some(
      (d: any) =>
        Number(d?.departmentId) === POST_BASIC_BSC_NURSING_ID ||
        d?.departmentName?.toLowerCase() === "post basic b.sc nursing"
    )
  );
    ...
    >
      ...
      >
        <h1 className="text-3xl md:text-5xl font-black text-white mb-3">
          Post Basic B.Sc Nursing Toppers Gallery
        </h1>
        ...
        <p className="text-white/70 mt-4 max-w-2xl mx-auto text-lg">
          Celebrating excellence in Post Basic B.Sc Nursing department.
        </p>
      </div>
  ...
  {loading ? (
    <div className="h-64 flex items-center justify-center">
      <Loader text="Loading Post Basic B.Sc Nursing toppers..." />
    </div>
  ) : postBasicToppers.length === 0 ? (
    <div className="text-center text-slate-600 text-sm py-20">
      No Post Basic B.Sc Nursing toppers found.
    </div>
  ) : (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
      {postBasicToppers.map((t) => (
        <div
          key={t.topperId}
