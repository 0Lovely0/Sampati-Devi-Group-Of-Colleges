  ...
  useEffect(() => {
    const fetchGalleries = async () => {
      try {
        ...
        console.error("Veterinary gallery fetch error:", error);
      } finally {
        setLoading(false);
      }
    };
            ...
            {galleryItems.length === 0 ? (
              <div className="bg-white rounded-3xl p-10 text-center shadow-lg max-w-3xl mx-auto">
                ...
                <p className="text-slate-500 mt-2">
                  No Veterinary Pharmacist gallery images are available at the moment.
                </p>
              </div>
            ) : (
              <>
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {galleryItems.slice(0, 10).map((item) => (
                    <div
                      ...
                      className="group relative h-32 overflow-hidden rounded-2xl bg-white border border-green-100 shadow-sm cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
                    >
                      <img
                ...
                <div className="mt-12 flex justify-center">
                  <Link
                    to="/programs/veterinary-pharmacist/gallery"
