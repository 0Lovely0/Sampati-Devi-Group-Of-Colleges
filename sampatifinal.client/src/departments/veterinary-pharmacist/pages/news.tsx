...
export const NewsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        ...
        if (vetNews.length > 0) {
          setSelectedNews(vetNews[0]);
        }
      } catch (error) {
        console.error("Veterinary news load error:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="h-screen flex items-center justify-center bg-[#DCFCE7]">
        <Loader text="Loading Veterinary Pharmacist news..." />
      </div>
    );
  }
  ...
  if (!selectedNews) {
    return (
      <div className="h-screen flex items-center justify-center bg-[#DCFCE7] text-[#15803D] font-semibold">
        No Veterinary Pharmacist news available
      </div>
    );
  }

  return (
    <div className="bg-[#DCFCE7] min-h-screen">
      {/* HEADER */}
      <div
      ...
      >
        <h1 className="text-4xl md:text-5xl font-black text-white">
          Veterinary Pharmacist News & Updates
        </h1>
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-6 rounded-2xl border transition-all duration-300 ${
                    active
                      ? "bg-[#15803D] border-[#15803D] shadow-xl"
                      : "bg-white border-[#BBF7D0] hover:border-[#22C55E] hover:shadow-md"
                  }`}
                >
                  <span
            ...
            <div className="bg-white p-8 md:p-12 rounded-3xl border border-[#BBF7D0] shadow-sm sticky top-28">
              ...
              {selectedNews.news_images && (
                <button
                  ...
                  className="bg-[#15803D] text-white px-8 py-3 rounded-full text-xs font-black uppercase tracking-widest hover:bg-[#22C55E] transition-all shadow-lg"
                >
                  View Attachment
                </button>
              )}
