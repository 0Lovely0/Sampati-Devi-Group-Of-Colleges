...
const FreeCounsellingPage: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // ✅ REAL API CALL (FILTER BY FORM TYPE)
      const res = await getApplyNowByFormType("FreeCounseling");
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div className="p-6">
          <h1 className="text-3xl font-bold text-slate-800">
            Free Counselling Enquiries
          </h1>

          <p className="text-slate-500 mt-1">
            Manage and review free counselling submissions.
          </p>
        </div>
          ...
          {!loading && filteredData.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <h3 className="text-lg font-semibold text-slate-700">
                No Free Counselling Enquiries Found
              </h3>

              <p className="text-sm text-slate-500 mt-1">
                New counselling enquiries will appear here.
              </p>
            </div>
          )}
